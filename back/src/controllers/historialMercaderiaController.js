...
export const registrarMovimientoMercaderia = async (req, res) => {
  const { auto_id, mercaderia_id, cantidad, fecha } = req.body;

  try {
    const [result] = await pool.query(
      `INSERT INTO historial_mercaderia (auto_id, mercaderia_id, cantidad, fecha) 
       VALUES (?, ?, ?, ?)`,
      [auto_id, mercaderia_id, cantidad || 1, fecha || new Date()],
    );
    res.status(201).json({
      id: result.insertId,
...
export const registrarMercaderiaTransaccion = async (
  ...
  if (!autoId || !mercaderiaId) {
    throw new Error(
      "Faltan datos obligatorios (auto_id o mercaderia_id) para registrar la mercadería.",
    );
  }
  ...
  const [result] = await connection.query(
    `INSERT INTO historial_mercaderia (auto_id, mercaderia_id, cantidad, fecha) 
     VALUES (?, ?, ?, NOW())`,
    [autoId, mercaderiaId, cantidad || 1],
  );
